    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        if (img) {
          ...
          ctx.fillStyle = `hsl(${(i * 360) / Math.max(1, this.items.length)}, 60%, 40%)`;
          ctx.fillRect(x, y, cellSize, cellSize);
          ctx.fillStyle = '#ffffff';
          ctx.font = 'bold 40px sans-serif';
          ctx.textAlign = 'center';
          ctx.textBaseline = 'middle';
          let title = this.items[i]?.title || 'Project';
          ctx.fillText(title.substring(0, 15), x + cellSize/2, y + cellSize/2);
        }
      });
